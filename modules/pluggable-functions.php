...
if ( !function_exists( 'wp_new_user_notification' ) ) :
	function wp_new_user_notification( $user_id, $deprecated = null, $notify = '' ) {
		...
		if ( $options['new_user_notification_to_admin'] == 'Y' ) {
			ds_new_user_notification_to_admin( $user_id, $deprecated, $notify );
		}
		if ( $options['ds_new_user_notification_to_user'] == 'Y' ) {
			ds_new_user_notification_to_user( $user_id, $deprecated, $notify );
		}
	}
endif;

function ds_new_user_notification_to_user( $user_id, $deprecated = null, $notify = '' ) {
	if ( $deprecated !== null ) {
...
function ds_new_user_notification_to_admin( $user_id, $deprecated, $notify = '' ) {
	if ( $deprecated !== null ) {
		_deprecated_argument( __FUNCTION__, '4.3.1' );
	}
	global $wpdb, $wp_hasher;
...
$options = ds_get_options();
if ( $options['ds_password_change_notification_to_admin'] == 'N' && !function_exists( 'wp_password_change_notification' ) ):
	/**
...
if ( !function_exists( 'dont_send_password_change_email' ) ):
	function dont_send_password_change_email( $send = false, $user = '', $userdata = '' ) {
		...
		if ( $options['ds_password_change_notification_to_admin'] == 'Y' ):
		...
		if ( $options['ds_password_change_notification_to_user'] == 'N' ):
			return false;
		else:
			return true;
		endif;
...
$options = ds_get_options();
if ( $options['ds_wp_notify_post_author'] == 'N' && !function_exists( 'wp_notify_postauthor' ) ):
	/**
...
if ( $options['ds_wp_notify_moderator'] == 'N' && !function_exists( 'wp_notify_moderator' ) ):
	/**
...
$options = ds_get_options();
if ( ( $options['ds_send_password_forgotten_email'] == 'N' ) && !function_exists( 'dont_send_password_forgotten_email' ) ):
	/**
	 * Email forgotten password notification to registered user.
	 *
	 */
	//echo "dont_send_password_forgotten_email off";exit;
	function ds_user_is_administrator( $user_id = 0 ) {
	...
	function dont_send_password_forgotten_email( $send = true, $user_id = 0 ) {
		$options = ds_get_options();
		$is_administrator = ds_user_is_administrator( $user_id );
		// if ( $is_administrator && empty( $famne_options['send_password_admin_forgotten_email'] ) ){
		//	 // stop sending admin forgot email
		//  return false;
		// }
		if ( !$is_administrator && $options['ds_send_password_forgotten_email'] == 'N' ) {
...
if ( $options['ds_auto_core_update_send_email'] == 'N' && !function_exists( 'ds_dont_sent_auto_core_update_emails' ) ):
	function ds_dont_sent_auto_core_update_emails( $send, $type, $core_update, $result ) {
		if ( !empty( $type ) && $type == 'success' ) {
			return false;
		}
		return true;
	}
	add_filter( 'auto_core_update_send_email', 'ds_dont_sent_auto_core_update_emails', 10, 4 );
endif;

if ( $options['ds_auto_plugin_update_send_email'] == 'N' ):
	function ds_auto_plugin_update_send_email( $notifications_enabled, $update_results_plugins ) {
	...
	add_filter( 'auto_plugin_update_send_email', 'ds_auto_plugin_update_send_email', 10, 2 );
endif;

if ( $options['ds_auto_theme_update_send_email'] == 'N' ):
	function ds_auto_theme_update_send_email( $notifications_enabled, $update_results_theme ) {
		$notifications_enabled = false;
	...
	add_filter( 'auto_theme_update_send_email', 'ds_auto_theme_update_send_email', 10, 2 );
endif;

if ( $options['ds_send_email_change_email'] == 'N' && !function_exists( 'dont_send_email_change_email' ) ):
	/**
